...
UiRender::UiRender(QWidget *parent) :
    QGLWidget(QGLFormat(QGL::DoubleBuffer | QGL::DirectRendering), parent),
    ui(new Ui::UiRender) {
    firstLaunch = true;
    texturesLoaded = true;
...
void UiRender::paintGL() {
    ...
    //Clear
    glClearColor(renderOptions->colors.value("empty").redF(), renderOptions->colors.value("empty").greenF(), renderOptions->colors.value("empty").blueF(), renderOptions->colors.value("empty").alphaF());
